...
const AddAppoinment = ({onSendAppointment,lastId})=>{
    ...
    function formDataPublish(){
        ...
        onSendAppointment(appointmmentInfo);
    ...
    return(
        <div className="mt-4">
            <button onClick={()=>setToggleForm(!toggleForm)} className={`bg-blue-400 text-white px-2 py-3 w-full text-left ${toggleForm?'rounded-t-md':'rounded-md'}`}>
                <div><BiCalendarPlus className="inline-block align-center"/>Add Appointment</div>
            </button>
            {
                toggleForm&&
                <div className="border-r-2 border-b-2 border-l-2 border-light-blue-500 rounded-b-md first-letter
                pl-4 pr-4 pb-4">
                    <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:pt-5">
                        ...
                        <div className="mt-1 sm:mt-0 sm:col-span-2 w-full">
                            ...
                            className="max-w-lg block w-full shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:max-w-sm border-gray-300 rounded-md" />
                        </div>
                    </div>
                    <div className="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:pt-5">
